...
async function main() {
    ...
    data: {
      ...
      references: {
        create: [
          {
            type: 'ACADEMIC_PAPER',
            title: 'The British Columbia Carbon Tax: A Review of the Latest "Grand Experiment" in Environmental Policy',
            author: 'Stewart Elgie and Jessica McClay',
            publication: 'McGill International Journal of Sustainable Development Law and Policy',
            url: 'https://example.com/bc-carbon-tax-study',
            publishedAt: new Date('2013-01-15'),
            notes: 'Demonstrates 5-15% emission reductions in BC from 2008-2012',
          },
          {
            type: 'STATISTICS',
            ...
            notes: 'Shows 70+ jurisdictions implementing carbon pricing mechanisms',
          },
        ],
      },
    },
    ...
    data: {
      ...
      references: {
        create: [
          {
            type: 'NEWS_ARTICLE',
            title: 'France\'s "Yellow Vest" Protesters Reject Macron\'s Climate Tax',
  ...
  await prisma.argumentQuote.create({
    data: {
      quotedText: 'may be regressive, disproportionately affecting low-income households',
        ...
        create: [
          {
            ...
            notes: 'Details revenue recycling mechanisms and distributional impacts',
          },
          {
            type: 'ACADEMIC_PAPER',
            title: 'Public Acceptability of Carbon Taxes: Lessons from British Columbia',
  ...
  const aiDebate = await prisma.debate.create({
    data: {
      title: 'Should AI development be subject to mandatory government licensing?',
      description:
  ...
  const ubiDebate = await prisma.debate.create({
    data: {
      title: 'Would Universal Basic Income reduce poverty more effectively than current welfare programs?',
      description:
  ...
  await prisma.report.create({
    data: {
      ...
      description: 'One of the cited sources appears to be a broken link and cannot be verified.',
      status: 'PENDING',
